    ...
    module SamlAuthenticatable
      ...
      module ClassMethods
        ...
        def authenticate_with_saml(attributes)
          ...
          if Devise.case_insensitive_keys.include? key
            auth_value.try(:downcase!)
          end

          resource = where(key => auth_value).first

          if resource.nil? && !Devise.saml_create_user
            logger.info("User(#{attributes[inv_attr[key.to_s]]}) not found.  "\
              'Not configured to create the user.')
            return nil
          end
        ...
        def attribute_map
          @attribute_map ||= YAML.load(
            File.read("#{Rails.root}/config/attribute-map.yml"))
        end
